...
def in_range_check(robot_state, line_obs):
    if abs(line_obs.points[0][0] - line_obs.points[1][0]) < 0.1:
        min_y = min(line_obs.points[0][1], line_obs.points[1][1]) - 0.5
        max_y = max(line_obs.points[0][1], line_obs.points[1][1]) + 0.5
        res = max_y > robot_state[1][0] > min_y
    else:
        min_x = min(line_obs.points[0][0], line_obs.points[1][0]) - 0.5
        max_x = max(line_obs.points[0][0], line_obs.points[1][0]) + 0.5
...
def dis_ratio(target, robot_state, line_obs):
    dist = np.linalg.norm(
        pf.shortest_distance_point(line_obs.points[0], line_obs.points[1], robot_state)[1] - robot_state)
    print(f"dist: {dist}")
    if dist == None:
        return 0
    return pow(10 / dist, 1)
...
for i in range(1000):
    ...
    for obs in line_obs:
        forces.append(pf.perpendicular(line_obstacle=obs.points, car_position=env.robot.state,
                                       coefficient=3 * dis_ratio(env.robot.goal, env.robot.state,
                                                                 obs) if in_range_check(
                                           env.robot.state, obs) else 0))
    ...
    if not run:
        for force in forces:
            f_repulsive += force
    dist = np.linalg.norm(env.robot.state - env.robot.goal)
    f_tangential = pf.tangential(point=env.robot.goal, car_position=env.robot.state,
                                 coefficient=1 * pow(dist, 0.5) if not run and dist > 5 else 0)
    f_attractive = pf.attractive(
        goal_point=env.robot.goal, car_position=env.robot.state, coefficient=0.6)
